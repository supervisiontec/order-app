function toggleActive(indexNo) {
    $.ajax({
        ...
        type: 'POST',
        datatype: 'json',
        url: "./rest/toggle-activation-customer/" + indexNo,
...
function deleteTransactor(indexNo) {
    $.ajax({
        ...
        type: 'POST',
        datatype: 'json',
        url: "./rest/delete-customer/" + indexNo,
        success: function (data, textStatus) {
            var component = $("#row-" + indexNo);

            if (data == true) {
                $.toaster({priority: "info", title: 'OK', message: "Customer deleted successfully !"});
                component.remove();
            }else{
                $.toaster({priority: "danger", title: 'ERROR', message: "Failed to delete customer !"});
            }
        }
    });
}